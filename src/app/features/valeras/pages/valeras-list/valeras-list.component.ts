...
import {
  ...
  CardConfig,
  CardAction,
} from '../../../../shared/components/card-grid/card-grid.component';
import {
  FormModalComponent,
  FormField,
} from '../../../../shared/components/form-modal/form-modal.component';
...
export class ValerasListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.realtime
      .on(RealtimeEvents.Valera.Created)
      .pipe(takeUntil(this.destroy$))
      .subscribe(reloadAll);
    this.realtime
      .on(RealtimeEvents.Valera.Updated)
      .pipe(takeUntil(this.destroy$))
      .subscribe(reloadAll);
    this.realtime
      .on(RealtimeEvents.Valera.Deleted)
      .pipe(takeUntil(this.destroy$))
      .subscribe(reloadAll);
    this.realtime
      .on(RealtimeEvents.Cliente.Created)
      .pipe(takeUntil(this.destroy$))
      .subscribe(reloadAll);
    this.realtime
    ...
    this.realtime
      ...
      .subscribe(reloadAll);
    this.realtime
      .on(RealtimeEvents.Consumo.Deleted)
      .pipe(takeUntil(this.destroy$))
      .subscribe(reloadAll);
  }
    ...
    forkJoin({
      ...
      .subscribe({
        next: ({ valeras, clientes }) => {
          ...
          clientes.forEach(
            (c) => c.clienteID && this.clientesMap.set(c.clienteID, c),
          );
  ...
  private applyFilter(): void {
    ...
    this.filteredValeras = this.valeras.filter(
      (v) =>
  ...
  private puedeConsumir(v: Valera): boolean {
    ...
    if (v.fechaVencimiento && new Date(v.fechaVencimiento) < new Date())
      return false;
          ...
          this.consumoService
            ...
            .subscribe({
              ...
              error: (e) => {
                ...
                const msg =
                  e?.error?.message ?? 'No se pudo registrar el consumo';
                if (isConflict) {
  ...
  openForm(valera?: Valera): void {
    ...
    modalRef.afterClose.subscribe((result) => {
      if (!result) return;
      const obs = isEdit
        ? this.valeraService.updateValera({
            ...result,
            valeraID: valera!.valeraID,
          })
      ...
      obs.subscribe({
        next: () => {
          this.notification.success(
            'Éxito',
            `Valera ${isEdit ? 'actualizada' : 'creada'} correctamente`,
          );
          this.loadAll();
        },
        error: () =>
          this.notification.error('Error', 'No se pudo procesar la solicitud'),
      });
    });
  }
